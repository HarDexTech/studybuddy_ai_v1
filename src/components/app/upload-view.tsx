...
import { UploadCloud, ListChecks, Baseline, MessageSquare, Loader2, CheckSquare, Timer, BrainCircuit, Brain, BrainCog, FileText, FileJson, FileType, History } from 'lucide-react';
...
type UploadViewProps = {
    ...
    existingDocument?: { text: string; file: { name: string, type: string, size: number } } | null;
};
...
function RecentDocuments({ onSelect }: { onSelect: (doc: CachedDocument) => void }) {
    ...
    return (
        <div className="mt-6">
            <h3 className="text-lg font-semibold mb-3 flex items-center gap-2"><History className="w-5 h-5"/> Recent Documents</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {recentDocs.map((doc) => (
                    <Card key={doc.id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => onSelect(doc)}>
...
export function UploadView({ onDocumentUploaded, onTestGenerated, existingDocument }: UploadViewProps) {
    ...
    const handleFileChange = async (selectedFile: File | null) => {
        ...
        if (selectedFile.size > MAX_FILE_SIZE) {
            handleError(`File size exceeds the 10MB limit. Please upload a smaller document.`, "File Too Large");
        ...
        try {
            ...
            let text = '';
             if (selectedFile.type === 'application/pdf') {
                ...
                const result = await mammoth.extractRawText({ arrayBuffer: arrayBufferResult });
                text = result.value;
            ...
            const fileInfo = { name: selectedFile.name, type: selectedFile.type, size: selectedFile.size };
            addRecentDocument({
                ...fileInfo,
    ...
    const handleSelectRecent = (doc: CachedDocument) => {
        onDocumentUploaded(doc.text, { name: doc.name, type: doc.type, size: doc.size });
    };
    ...
    const handleGenerateTest = async () => {
        if (!existingDocument) {
            toast({ variant: 'destructive', title: "No document context", description: "Something went wrong. Please upload the document again." });
            return;
        }
        ...
        const questionsToGenerate = useBatching ? INITIAL_BATCH_SIZE : settings.numberOfQuestions;
        
        setLoadingMessage(useBatching ? `Generating initial batch of ${questionsToGenerate} questions...` : 'Warming up the AI...');

        const generatedQuestions: Question[] = [];
        try {
            for (let i = 0; i < questionsToGenerate; i++) {
                if (!useBatching) {
                    setLoadingMessage(`Generating question ${i + 1} of ${settings.numberOfQuestions}...`);
                }
            ...
            if (generatedQuestions.length > 0) {
                ...
                handleError("The AI couldn't generate a test from this document. Please try another one.");
            }
        } catch (error) {
            console.error(error);
            ...
            <div className="w-full max-w-2xl mx-auto flex-grow flex items-center">
                 <Card className="w-full animate-in fade-in-50 duration-500">
                    <CardHeader>
                        ...
                        <CardDescription className="text-center">Adjust the settings for the test based on your document: <span className="font-semibold text-primary">{existingDocument.file.name}</span></CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={settings.questionType} onValueChange={(value) => setSettings({ ...settings, questionType: value as TestSettings['questionType'] })}>
                                <SelectTrigger id="question-type" className="w-full">
                                    <SelectValue placeholder="Select type" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="multiple choice"><div className="flex items-center gap-2"><ListChecks className="w-4 h-4" /> Multiple Choice</div></SelectItem>
                                    <SelectItem value="fill-in-the-blank"><div className="flex items-center gap-2"><Baseline className="w-4 h-4" /> Fill-in-the-Blank</div></SelectItem>
                                    <SelectItem value="theory"><div className="flex items-center gap-2"><MessageSquare className="w-4 h-4" /> Open-Ended</div></SelectItem>
                                    <SelectItem value="true or false"><div className="flex items-center gap-2"><CheckSquare className="w-4 h-4" /> True or False</div></SelectItem>
                                    <SelectItem value="all"><div className="flex items-center gap-2"><FileType className="w-4 h-4" /> All Types</div></SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={(e) => setSettings({ ...settings, numberOfQuestions: Math.min(50, Math.max(1, parseInt(e.target.value, 10) || 1)) })}
                                min="1"
                                max="50"
                                className="w-full"
                            />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            ...
                            <Select value={settings.difficulty} onValueChange={(value) => setSettings({ ...settings, difficulty: value as TestSettings['difficulty'] })}>
                                <SelectTrigger id="difficulty" className="w-full">
                                    <SelectValue placeholder="Select difficulty" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="easy"><div className="flex items-center gap-2"><BrainCog className="w-4 h-4" /> Easy</div></SelectItem>
                                    <SelectItem value="medium"><div className="flex items-center gap-2"><Brain className="w-4 h-4" /> Medium</div></SelectItem>
                                    <SelectItem value="hard"><div className="flex items-center gap-2"><BrainCircuit className="w-4 h-4" /> Hard</div></SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                        <div className="space-y-2">
                             <Label htmlFor="question-source">Question Source</Label>
                            <Select value={settings.questionSource} onValueChange={(value) => setSettings({ ...settings, questionSource: value as TestSettings['questionSource'] })}>
                                <SelectTrigger id="question-source" className="w-full">
                                    <SelectValue placeholder="Select source" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="strict"><div className="flex items-center gap-2"><FileText className="w-4 h-4" /> Strictly from Text</div></SelectItem>
                                    <SelectItem value="formed"><div className="flex items-center gap-2"><FileJson className="w-4 h-4" /> Formed from Concepts</div></SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                    </div>
                     <div className="space-y-4">
                        <div className="flex items-center space-x-2">
                            <Switch id="timer-enabled" checked={settings.timerEnabled} onCheckedChange={(checked) => setSettings({ ...settings, timerEnabled: checked })}/>
                            <Label htmlFor="timer-enabled" className="flex items-center gap-2"><Timer className="w-4 h-4" /> Enable Timer</Label>
                        </div>
                        {settings.timerEnabled && (
                            <div className="space-y-2 pl-8 animate-in fade-in-50 duration-300">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setSettings({ ...settings, timerDuration: parseInt(e.target.value, 10) || 1 })}
                                    min="1"
                                />
                            </div>
                        )}
                        ...
                        <div className="flex flex-col items-center gap-2 text-muted-foreground">
                            ...
                            {file && !isParsing ? (
                                ...
                                <>
                                    ...
                                    <p className="text-xs mt-2">PDF, DOCX, PPTX, TXT supported (Max 10MB)</p>
                                </>
                            )}
                        </div>
                    </div>
