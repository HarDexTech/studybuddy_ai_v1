'use server';
/**
 * @fileOverview A flow to generate a single test question with automatic
 * provider fallback (primary NIM/DeepSeek, fallback Gemini — or inverted via
 * AI_PROVIDER=gemini).
 */
...
const SingleTestQuestionSchema = z.object({
...
export type GenerateSingleTestQuestionOutput = z.infer<typeof SingleTestQuestionSchema>;

// Build prompt text
function buildPrompt(input: GenerateSingleTestQuestionInput): {
...
You MUST set the 'type' field in your response to the chosen question type.`
    : `You MUST generate a '${selectedType}' question. You should NOT include a 'type' field in your response.`
}
...
**GROUNDING RULES:**
- Only generate a question whose answer is present in or directly derivable from the Document Content below. Do NOT ask a question that relies on outside knowledge or that the document cannot answer.
- If the document is a past-question or question-only sheet without explicit answers, base the question on the topics the document covers and ensure the answer can be reasoned from the document (or the question itself).
...
// Parse response into the single-question shape
function parseResponse(rawResponse: string): GenerateSingleTestQuestionOutput {
  try {
    ...
    console.error('Failed to parse single-question response:', error);
...
export async function generateSingleTestQuestion(input: GenerateSingleTestQuestionInput): Promise<GenerateSingleTestQuestionOutput> {
  ...
  const result = await callJson<GenerateSingleTestQuestionOutput>(
    systemInstruction,
    userPrompt,
  ...
  const normalizedResult = shuffleMultipleChoiceChoices(result as GenerateSingleTestQuestionOutput);
